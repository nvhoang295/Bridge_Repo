...
void SortingAlgor::readFile(string fileName, vector<SortingAlgor> &data) {
	ifstream file(fileName, ios_base::in);
	string line;
	while (!file.eof()) {
		getline(file, line);
		...
		bool isStable = stoi(propVals[5].c_str());
...
void FileController::writeFile(string fileName, vector<Student> &students, int size) {
	...
	for (int i = 0; i < size; ++i) {
		...
		file << students[i].getMajorId() << "|";
		file << students[i].getMajorName() << "|";
		file << students[i].getFirstName() << "|";
		file << students[i].getLastName() << "|";
		file << students[i].getGender() << "|";
		file << students[i].getDob() << "\n";
	}
	file.close();
	students.clear();
